...
const SupplierDashboard = () => {
  const { orders, fetchSupplierOrders } = useOrderStore()
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                          {order.items.map((item, index) => (
                            <div key={index} className="bg-gray-50 p-2 rounded text-sm">
                              <span className="font-medium">{item.quantity} {item.unit} {item.item}</span>
                              {item.price_per_unit && (
                                <span className="text-gray-600 ml-2">@ ₹{item.price_per_unit}</span>
                              )}
                            </div>
                          ))}
                        </div>
